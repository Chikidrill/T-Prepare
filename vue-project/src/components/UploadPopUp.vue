    ...
    <div class="upload-popup popup">
      <div class="upload-popup__content">
        ...
        <div class="upload-popup__buttons">
          <button class="upload__btn btn" @click="uploadToServer">Загрузить в формат QuizAI</button>
          <button class="upload__btn-alt btn">Загрузить в формат самопроверки</button>
        </div>
      </div>
    </div>
  </template>
  ...
  export default {
    data() {
      return {
        ...
        fileData: null, // Хранение файла
        jsonData: null, // Хранение JSON-данных
      };
    },
        ...
        if (file && (file.type === "application/json" ||
          ...
          reader.onload = (e) => {
            if (file.type === "application/json") {
              ...
              this.fileData = file; // Просто сохраняем файл
              this.$emit('add-subject', this.subjectName, this.fileData);
            }
          };
          ...
          if (file.type === "text/plain") {
            ...
            this.fileData = file; // Просто сохраняем объект File
          }
        } else {
  ...
  if (this.subjectName && this.fileData) {
    try {
      ...
      if (response.ok) {
        ...
        // Обрабатываем данные и передаем их через событие
        this.jsonData = result;
        this.$emit('add-subject', this.subjectName, this.jsonData);

        // Закрываем popup
        this.$emit('close'); // Закрываем попап после успешной загрузки
...
mounted() {
  const storedData = localStorage.getItem('uploadedJsonData');
  if (storedData) {
    this.selectedSubject = JSON.parse(storedData);
  }
}
